...
pipenv run python manage.py startapp ${PROJECT_NAME}api
mkdir ./.vscode
...
python manage.py makemigrations ${PROJECT_NAME}api
python manage.py migrate ${PROJECT_NAME}api
...
pipenv run bash -c "python manage.py migrate"
git init
git add --all
